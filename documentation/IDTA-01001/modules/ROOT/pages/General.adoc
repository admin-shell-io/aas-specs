...
====
Note: the NIST model xref:IDTA-01xxx_Bibliography.adoc#bib49[[49\]] provides an illustration of the second/third class of relationships.
====
...
Details and examples for composite I4.0 Components can be found in xref:IDTA-01xxx_Bibliography.adoc#bib12[[12\]]. A composite I4.0 Component is the combination of a complex asset and its Asset Administration Shell. The hierarchy, typically a Bill of Material (BOM) but also any other relationship between different assets, can be represented in one of its submodels.
...
According to xref:IDTA-01xxx_Bibliography.adoc#bib4[[4\]], identifiers are needed for the unique identification of many different elements within the domain of smart manufacturing. They are a fundamental element of a formal description of the Administration Shell. Identification is especially required for
...
In the domain of smart manufacturing, the assets need to be uniquely identified worldwide xref:IDTA-01xxx_Bibliography.adoc#bib4[[4\]] xref:IDTA-01xxx_Bibliography.adoc#bib20[[20\]] by the means of identifiers (IDs). The Administration Shell also has a unique ID (see Figure 5).

.Unique Identifier for Administration Shell and Asset (Modified Figure from xref:IDTA-01xxx_Bibliography.adoc#bib4[[4\]])
image::image6.jpeg[]

An Administration Shell represents exactly one asset, with a unique asset ID. In a batch-based production, the batches will become the assets and will be described by a respective Administration Shell. If a set of assets shall be described by an Administration Shell, a unique ID for the composite asset needs to be created xref:IDTA-01xxx_Bibliography.adoc#bib13[[13\]].

The ID of the asset needs to comply with the restrictions for global identifiers according to xref:IDTA-01xxx_Bibliography.adoc#bib4[[4\]]xref:IDTA-01xxx_Bibliography.adoc#bib20[[20\]]. If the asset features further identifications like serial numbers and alike, they are not to be confused with the unique global identifiers of the asset itselffootnote:[Such additional asset identifiers are contained in _AssetInformation/specificAssetIds_.].

=== What Type of Identifiers Exist?

In xref:IDTA-01xxx_Bibliography.adoc#bib4[[4\]]xref:IDTA-01xxx_Bibliography.adoc#bib20[[20\]], two standard-conforming global identification types are defined:

* *IRDI* – ISO29002-5, ISO IEC 6523 and ISO IEC 11179-6 xref:IDTA-01xxx_Bibliography.adoc#bib20[[20\]] as an identifier scheme for properties and classifications. They are created in a process of consortium-wise specification or international standardization. To this end, users come together and feed their ideas into the consortia or standardization bodies. Properties in ISO, IEC help to safeguard key commercial interests. Repositories like ECLASS and others make it possible to standardize a relatively large number of identifiers in an appropriately short time.
...
See also DIN SPEC 91406 xref:IDTA-01xxx_Bibliography.adoc#bib31[[31\]] for further information on identification.
...
The Administration Shell fosters the use of worldwide unique identifiers to a large degree. However, in some cases, this may lead to inefficiencies. Example: a property, which is part of a submodel, which in turn is part of an Administration Shell, each of which is identified by global identifiers xref:IDTA-01xxx_Bibliography.adoc#bib4[[4\]].
...
In November 2020, the first two submodel templates for the Asset Administration Shell were published, one for a nameplate xref:IDTA-01xxx_Bibliography.adoc#bib40[[40\]] and one for generic technical data xref:IDTA-01xxx_Bibliography.adoc#bib39[[39\]]. Others followed and will follow. Please see xref:IDTA-01xxx_Bibliography.adoc#bib45[[45\]] for an overview of registered submodel templates.
