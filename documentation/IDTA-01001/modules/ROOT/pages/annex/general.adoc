...
====
Note 1: to distinguish asset 'type' and asset 'instance', the term 'asset kind' is used in this document.
...
====
Note 2: the NIST model xref:bibliography.adoc#bib49[[49\]] provides an illustration of the second/third class of relationships.
====
...
====
Note 3: for submodels representing the Bill of Material of a complex asset, the metamodel not only provides the possibility to define relationships (via the submodel element xref:spec-metamodel/submodel-elements.adoc#relationship-element-attributes[RelationshipElement], see above), 
...
====
Note 1: the example is simplified for ease of understanding and only roughly complies with the metamodel as specified in Clause xref:spec-metamodel/index.adoc[].
...
The metamodel for Asset Administration Shells is defined in Clause xref:spec-metamodel/index.adoc#part1-specification[Specification].
...
image::6-unique-identifier-for-aas-and-asset.jpeg[pdfwidth=50%,scaledwidth=50%]
