...
The approach for semantics and interaction for I4.0 components xref:bibliography.adoc#bib18[[18\]] suggests the use of the following structure (see Table <<table-proposed-structure-for-uris>>) for URIsfootnote:[URLs are also URIs], which is slightly modified here.
...
Examples of such identifiers are given in Table 12.
