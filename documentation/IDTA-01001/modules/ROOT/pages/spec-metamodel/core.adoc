...
For more detailed information on the _derivedFrom_ concept, see Clause xref:annex/general.adoc#types-and-instances[Types and Instances].
...
h|Explanation: 3+a|
In _AssetInformation,_ identifying metadata of the asset that is represented by an xref:AssetAdministrationShell[Asset Administration Shell] is defined.
...
.2+e|[[specificAssetId]]specificAssetId 3+| `\https://admin-shell.io/aas/3/1/AssetInformation/specificAssetIds`
...
But it is also possible to have an _AssetInformation/assetType_ of an asset of xref:AssetInformation#assetKind[kind] kind xref:AssetKind[Type].
====
...
.2+e|Instance | `\https://admin-shell.io/aasgit gq
git/3/1/AssetKind/Instance`
a|Instance asset
...
For more information on types and instances, see Clause xref:annex/general.adoc#types-and-instances[Types and Instances].
