...
Subclause xref:general.adoc#types_and_instances[Types and Instances] describes some general aspects of handling type and instance assets.

Subclause xref:general.adoc#identification_of_elements[Identification of Elements] explains the very important aspects of identification in the context of the Asset Administration Shell.

Subclause xref:general.adoc#matching_strategies[Matching Strategies] provides matching strategies for semantic identifiers and references.

Subclause xref:general.adoc#submodel_instances_and_templates[Submodel Instances and Templates] explains the difference between submodel instances and templates.
...
Not every identifier is applicable for every element of the UML model representing the Asset Administration Shell. Table <<table-elements-with-allowed-identifying-values>> therefore gives an overview on the different constraints and recommendations on the various entities, which implement xref:spec-metamodel/common.adoc#identifiable_attrs[Identifiable] or xref:spec-metamodel/common.adoc#has_semantics_attrs[HasSemantics].

See Annex xref:annex/concepts-aas.adoc#how_are_new_identifiers_created[How Are New Identifiers Created?] for more information on how to create new identifiers and best practices for creating URI identifiers.
...
(GlobalReference)https://example.com/aas/1/1/1234859590, (FragmentReference)Specification, (FragmentReference)Bibliography
....
[#submodel_instances_and_templates]
