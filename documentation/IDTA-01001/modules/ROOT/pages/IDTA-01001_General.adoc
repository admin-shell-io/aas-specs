...
For examples used to illustrate valid and invalid matching candidates, the string serialization of references as defined in Clause 7.2.3 is used for easier understanding.

The definition of link:https://www.w3.org/TR/xmlschema-2/#terminology[XML Schema] is used for matching string values:
...
[[matching-strategies-for-refences]]
=== Matching Strategies for References

 This clause explains matching strategies based on the reference concept (see Clause 5.3.10), 
 i.e. for two elements with type xref:IDTA-01001_Metamodel_Referencing.adoc#Reference[Reference]. 
 clause  xref:matching-strategies-for-semantic-identifiers[] additional hints for matching 
 strategies with respect to semantic IDs are given.


**Exact matching of two references**

====
Definition Exact-Matching: Two References are matching if all attribute values match.
====

====
Note 1: The definition of axact-matching includes that the values of the Key/type attributes match and not only the Key/value values.
====

====
Note 2: The definition of axact-matching includes that also the values for the Reference/type and Reference/referredSemanticId match. The latter again is a value of type xref:IDTA-01001_Metamodel_Referencing.adoc#Reference[Reference] and the same rules apply.
====

**Value matching of two references**

====
Definition Value-Matching: Two References are matching if all values of the keys, i.e. the values of Key/value match.
====

====
Note 1:
The Reference/type and Reference/referredSemanticId are ignored for value-matching. This means that an external reference can also match a model reference.
====

====
Note 2: the key type can be ignored since the fragment keys are always unique (e.g. all idShorts of submodel elements in a submodel or all submodel elements in a submodel element list or collection).
====

====
Note 3: the Reference/referredSemanticId can be ignored because it does not serve identifiying purposes.
====

====
Note 4: the case that a fragement value matches a global reference value is considered to be unlikely: this risk of mismatch is accepted in the matching strategy. that a fragment value is identical to a global reference value.
====


====
Note 5: since identifiables of the Asset Administration Shell are globally unique, model references are special cases of global references. The only difference is the handling of key types that are predefined for Asset Administration Shell elements. Other key types could be predefined, e.g. for IRDI paths etc. However, so far only generic key types are supported.
====

====
Note 6: If the values for attribute "referredSemanticId" of the two references compared do not match then there is a mismatch between the two that should be resolved. However, the two are considered to value-match in the context of discovery.
====


====
Note 7: if the key type values (i.e. value of Key/type) do not match although all key values follow the correct order of the key chain, then at least one of the references is buggy and a warning should be issued.
====
...
Semantic identifiers are a special case of elements with type xref:Reference[], see clause xref:matching-strategies-for-refences[].
