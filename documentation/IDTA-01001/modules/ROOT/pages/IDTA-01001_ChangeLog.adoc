...
|   | xref:Spec/IDTA-01001_Metamodel_SubmodelElements.adoc#RelationshipElement[RelationshipElement/first] a| now optional
|   | xref:Spec/IDTA-01001_Metamodel_SubmodelElements.adoc#RelationshipElement[RelationshipElement/second] a| now optional
g
|===
...
* CHANGE: DataTypeDefXsd all links now reference correctly to XML Schema 1.0 (xmlschema-2) not to XML Schema V1.1 (xmlschema11-2) (https://github.com/admin-shell-io/aas-specs/issues/312[#312], https://github.com/admin-shell-io/aas-specs/issues/363[#363]) + editorial changes (remove redundant entries, sort entries), schema: Check for xs:anyUri updated from [RFC 3987] to [RFC 2396]. Additionally, added notes that RDF is using XML Schema 1.1 but  a subset.
...
* (Editorial) correct Figure Metamodel of an AssetAdministrationShell: arrow removed from association between Submodel and AssetAdministrationShell
