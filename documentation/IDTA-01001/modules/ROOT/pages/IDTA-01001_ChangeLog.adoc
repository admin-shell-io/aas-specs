...
* PathType: support complete URI scheme for files RFC2396 conformant to anyURI of XML Schema 1.0 (https://github.com/admin-shell-io/aas-specs/issues/299[#299])
...
|   | PathType a| extend data type to support complete URI scheme for files RFC2396
...
|===
|*Nc* |*V3.1 Change w.r.t. V3.0* |*Comment*
|(x)| BasicEventElement/Direction a| (bugfix in Text only) attribute starts with small letter 
|(x) | BasicEventElement/State a|  (bugfix in Text only) attribute starts with small letter 
|x| DataTypeDefXsd a| BUGFIX: Remove xs:yearMonthDuration from enumeration

Updated links to XML Schema 1.0 (not 1.1)
|(x)| EmbeddedDataSpecification/dataSpecification a| (bugfix in Text only) now mandatory
|(x) | MultiLanguageNameType a| (bugfix in Text only) maxLength=128 (as derived and implemented)
|(x)  | Qualifier/Kind a| (bugfix in Text only) attribute starts with small letter 
|===
